...
const getLocation = () => {
  ...
  locationBtn.addEventListener('click', (e) => {
    ...
    getGeoLocation().then(pos => {
      ...
      const urlGeoPos = `https://geocode.xyz/${lat},${lng}?geoit=json`;
      return getResource(urlGeoPos).then(Gdata => {
        console.log(Gdata);
        console.log(Gdata.osmtags.name_cs);
        const urlByName = `http://api.openweathermap.org/data/2.5/weather?q=${Gdata.osmtags.name_cs}&units=metric&appid=2fbb20e9e1081a0b869a449bc5eb6e87`;
        getResource(urlByName).then(data => {
          console.log(data);
          resetData();
    ...
    `;
          ...
          moreDaysweather.addEventListener('click', (e) => {
            ...
            getResource(URLMoreDays).then(function (mData) {
              console.log(mData);
              if (weatherInfoContainer.firstElementChild) {
                weatherInfoContainer.firstElementChild.remove();
              }
              if (weatherMoreContainer.firstElementChild) {
                weatherMoreContainer.firstElementChild.remove();
              }
